        ...
        <div style={{ flex: 1, overflowY: "auto", padding: isMobile ? "12px 10px" : "14px 20px", background: C.bg }}>
          ...
          {tab === "fin" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10 }}>
              {[["P/E", d.pe || "—", "주가수익비율"], ["P/B", d.pb || "—", "주가순자산비율"], ["EPS", "$" + (d.eps || "—"), "주당순이익"], ["ROE", fund.roe != null ? fund.roe + "%" : "—", "자기자본이익률"], ["D/E", fund.de != null ? fund.de + "x" : "—", "부채비율"], ["유동비율", fund.currentRatio != null ? fund.currentRatio + "x" : "—", "단기 지급능력"], ["매출", fund.rev || "—", "연간 매출"], ["영업이익", fund.opInc || "—", "연간 영업이익"], ["매출총이익률", fund.grossMargin != null ? fund.grossMargin + "%" : "—", "Gross Margin"], ["순이익률", fund.netMargin != null ? fund.netMargin + "%" : "—", "Net Margin"]].map(([k, v, desc]) => <div key={k} style={{ ...card, display: "flex", justifyContent: "space-between", alignItems: "center" }}><div><div style={{ fontSize: 11, color: C.m, fontWeight: 600 }}>{k}</div><div style={{ fontSize: 10, color: C.sub, marginTop: 1 }}>{desc}</div></div><div style={{ fontSize: 20, fontWeight: 800, color: C.t }}>{v}</div></div>)}
              </div>
            </div>
          )}
